...
function volcanoPlot() {
    ...
    function chart(selection){
        ...
        selection.each(function(data) {
            ...
            function zoomFunction() {
                ...
                d3.selectAll('.dot.'+unique_id)
                    .attr('transform', transform)
                    .attr('r', 3 / Math.sqrt(transform.k));
                gX.call(xAxis.scale(d3.event.transform.rescaleX(xScale)));
                gY.call(yAxis.scale(d3.event.transform.rescaleY(yScale)));
                svg.selectAll('.threshold.'+unique_id)
                    .attr('transform', transform)
                    .attr('stroke-width', 1 / transform.k);
            }

            function circleClass(d) {
                if (d[yColumn] <= significanceThreshold && Math.abs(d[xColumn]) >= foldChangeThreshold) return 'dot sigfold '+unique_id;
                else if (d[yColumn] <= significanceThreshold) return 'dot sig '+unique_id;
                else if (Math.abs(d[xColumn]) >= foldChangeThreshold) return 'dot fold '+unique_id;
                else return 'dot '+unique_id;
            }
    ...
    chart.unique_id = function(value) {
        if (!arguments.length) return unique_id;
        unique_id = value;
        return chart;
    };
